...
```bash
mv ~/nexus/index ~/nexus/index_sauvegarde_L1
```

**3. Remplacer les anciens cours par les nouveaux**
```bash
rm -rf ~/nexus/docs/*
...
> 💡 Pour revenir à l'ancien index, supprimez `~/nexus/index/` et remplacez-le par votre sauvegarde : `mv ~/nexus/index_sauvegarde_L1 ~/nexus/index`
...
Les modèles disponibles sont hébergés sur Google Drive :  
👉 **[Télécharger les modèles](https://drive.google.com/drive/folders/1dMfF8h54zyKeSEQLq4mNDjvkMn-8H8CF?usp=sharing)**

Une fois le modèle téléchargé (fichier `.gguf`) :

```bash
# Créez un sous-dossier du nom du modèle et placez-y le fichier
# Exemple pour Qwen :
mkdir -p ~/nexus/model/qwen
mv /chemin/vers/le/fichier.gguf ~/nexus/model/qwen/

# Exemple pour Phi :
mkdir -p ~/nexus/model/phi
mv /chemin/vers/le/fichier.gguf ~/nexus/model/phi/
...
> 💡 Vous pouvez télécharger un seul modèle ou plusieurs selon vos besoins. Chaque modèle offre un compromis entre vitesse et qualité des réponses.
